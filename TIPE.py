...
def start(clas):
    ...
    if not hasattr(clas, "get") or not callable(clas.get):
        raise ValueError("Il faut une fonction get(t) définie dans "+clas.__name__)
    ...
    for name in filter(lambda x: not x.startswith('__'), dir(clas)):
        ...
        if type(getattr(clas, name)) in [float, int]:
            ...
            print("Info : {} was detected to be a {} of value {}".format(name, type(getattr(clas, name)), getattr(clas, name)))
